...
namespace RAMSDB_WinForms
{
    public partial class MapViewer : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        public MapViewer()
        {
            ...
            //aBoundary = dataLoader.roadsPerUserDL.Where(x=>x.AdminBoundary.RegionName=="SNNP").Select(x => x.AdminBoundary).ToList();

            string _region = RAMSDBDataLoader.LoggedInUser.userRegion;
            aBoundary = _context.AdminBoundary.Where(c => c.RegionName == _region).ToList();
        ...
        public MapViewer(string roadType)
        {
            ...
            if (roadType == "Unpaved")
            {
                roadsPerUser = dataLoader.unpavedRoadsPerUserDL;

                rGIS = dataLoader.unpavedRoadsPerUserDL.Select(x => x.RoadsGISData).ToList();
            }
            else if (roadType == "Paved")
            {
                roadsPerUser = dataLoader.pavedRoadsPerUserDL;
                rGIS = dataLoader.pavedRoadsPerUserDL.Select(x => x.RoadsGISData).ToList();
            }
            ...
            if (woredasGeomItemStorage !=null ) layerWoreda.Data = woredasGeomItemStorage;

            if (roadGeomItemStorage != null) layerRoads.Data = roadGeomItemStorage;
